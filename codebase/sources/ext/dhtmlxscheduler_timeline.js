		...
		if (this.render == "cell"){
			for (var j=0; j < scheduler._cols.length; j++) {
				if (scheduler._ignores[j])
					...
					html+="<td class='dhx_matrix_cell "+scheduler.templates[this.name+"_cell_class"](evs[i][j],this._trace_x[j],this.y_unit[i])+"' style='width:"+(scheduler._cols[j])+"px'><div style='width:auto'>"+scheduler.templates[this.name+"_cell_value"](evs[i][j],this._trace_x[j],this.y_unit[i])+"</div></td>";
			}
		} else {
			//section 2
			...
			for (var j=0; j < scheduler._cols.length; j++){
				if (scheduler._ignores[j])
					...
					html+="<td class='dhx_matrix_cell "+scheduler.templates[this.name+"_cell_class"](evs[i],this._trace_x[j],this.y_unit[i])+"' style='width:"+(scheduler._cols[j])+"px'></td>";
			}
			html+="</table>";
			html+="</div></td>";
		}
...
scheduler._matrix_tooltip_handler = function(e){
	...
	if (obj){
		...
		if (pos)
			return scheduler._timeline_showToolTip(obj,pos,getOffset(pos.src));
	}
	scheduler._timeline_hideToolTip();
};
